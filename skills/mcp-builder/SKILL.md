---
name: mcp-builder
description: 构建 MCP (Model Context Protocol) 服务器，为 Claude 赋予新能力。适用于当用户想要创建 MCP 服务器、为 Claude 添加工具或集成外部服务时。
---

# MCP 服务器构建技能

你现在具备构建 MCP (Model Context Protocol) 服务器的专业知识。MCP 使 Claude 能够通过标准化协议与外部服务进行交互。

## 什么是 MCP？

MCP 服务器暴露：
- **工具 (Tools)**：Claude 可以调用的函数（如 API 端点）
- **资源 (Resources)**：Claude 可以读取的数据（如文件或数据库记录）
- **提示词 (Prompts)**：预构建的提示词模板
