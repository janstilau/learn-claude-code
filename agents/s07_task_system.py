...
任务作为 JSON 文件持久化在 .tasks/ 中，因此它们可以在上下文压缩后存活。
...
SYSTEM = f"你是一个位于 {WORKDIR} 的编码代理。使用任务工具来计划和跟踪工作。"


# -- TaskManager: 带有依赖图的 CRUD，持久化为 JSON 文件 --
class TaskManager:
    ...
    def list_all(self) -> str:
        ...
        for t in tasks:
            ...
            blocked = f" (被以下任务阻塞: {t['blockedBy']})" if t.get("blockedBy") else ""
...
TOOLS = [
    ...
    {"name": "task_get", "description": "通过 ID 获取任务的完整详细信息。",
...
if __name__ == "__main__":
    ...
    while True:
        try:
            logtool.print_user("")
            query = input(f"{logtool.Colors.CYAN}s07 >> {logtool.Colors.RESET}")
